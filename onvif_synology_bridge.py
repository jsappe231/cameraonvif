...
def connect_pullpoint(config: Config) -> Any:
    ...
    subscription_xaddr = normalize_xaddr(subscription_xaddr, config)
...
def normalize_xaddr(xaddr: str, config: Config) -> str:
    parsed = urlparse(xaddr)
    if parsed.scheme and parsed.netloc:
        return xaddr
    if xaddr.startswith("/"):
        return f"http://{config.camera_host}:{config.camera_port}{xaddr}"
    return xaddr
...
def find_matching_name(raw: Any, patterns: tuple[str, ...]) -> str | None:
    if isinstance(raw, Mapping):
        simple_item_value = matching_simple_item_value(raw, patterns)
        if simple_item_value:
            return simple_item_value
        ...
        for item in raw:
            ...
            if nested:
                return nested
    elif isinstance(raw, str) and matches_pattern(raw, patterns):
        return raw
    return None


def matching_simple_item_value(raw: Mapping[Any, Any], patterns: tuple[str, ...]) -> str | None:
    name = raw.get("Name")
    value = raw.get("Value")

    for candidate in (value, name):
        if isinstance(candidate, str) and matches_pattern(candidate, patterns):
            return candidate
    return None


def matches_pattern(value: str, patterns: tuple[str, ...]) -> bool:
